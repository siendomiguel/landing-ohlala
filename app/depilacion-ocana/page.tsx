...
export const metadata: Metadata = {
  title: 'Depilación en Ocaña Centro - Depilación Profesional',
  description:
    'Depilación profesional en Ocaña centro. Depilación de bozo, barba, espalda, piernas, abdomen y zona púbica. Calle 10 #10-19. Agenda tu cita por WhatsApp.',
  alternates: {
    canonical: '/depilacion-ocana',
  },
  openGraph: {
...
const servicios = [
  ...
  'Depilación de abdomen',
  'Depilación zona púbica',
];

export default function DepilacionOcana() {
  return (
    <>
      <Script id="depilacion-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      ...
      <div className="flex flex-col space-y-6 w-full pb-8">
        <div className="text-center bg-white/80 backdrop-blur-sm text-purple-800 py-4 px-8 rounded-2xl shadow-sm border border-white/50">
          <h2 className="text-xl font-bold">Depilación en Ocaña Centro</h2>
          <p className="text-purple-600 text-sm mt-1">Servicio profesional de depilación</p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-md border border-white/50">
          <p className="text-gray-800 font-medium text-center leading-relaxed">
            En <strong className="text-purple-800">Ohlala Estudio de Belleza</strong> ofrecemos
            depilación profesional en diferentes zonas del cuerpo. Técnicas seguras y efectivas
            para una piel suave y libre de vello.
          </p>
        </div>
        ...
        <Link href="https://wa.me/573212600682?text=Hola%2C%20quiero%20agendar%20una%20cita%20de%20depilación" target="_blank" className="block group">
