...
export const metadata: Metadata = {
  title: 'Estilista de Cabello en Ocaña Centro - Tintes y Tratamientos Capilares',
  description:
    'Estilista de cabello en Ocaña centro. Tinte de cabello, tratamientos capilares, hidratación y queratina profesional. Calle 10 #10-19. Agenda tu cita por WhatsApp.',
  alternates: {
    canonical: '/cabello-ocana',
  },
  openGraph: {
...
const servicios = [
  'Tinte de un solo color',
  'Tratamientos capilares',
  'Hidratación del cabello',
  'Keratina',
];

export default function CabelloOcana() {
  return (
    <>
      <Script id="cabello-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      ...
      <div className="flex flex-col space-y-6 w-full pb-8">
        <div className="text-center bg-white/80 backdrop-blur-sm text-purple-800 py-4 px-8 rounded-2xl shadow-sm border border-white/50">
          <h1 className="text-xl font-bold">Cabello en Ocaña Centro</h1>
          <p className="text-purple-600 text-sm mt-1">Tintes y tratamientos capilares profesionales</p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-md border border-white/50">
          <p className="text-gray-800 font-medium text-center leading-relaxed">
            En <strong className="text-purple-800">Ohlala Estudio de Belleza</strong> cuidamos tu
            cabello con tratamientos profesionales. Tintes de calidad, hidratación profunda y
            queratina para un cabello sano y brillante.
          </p>
        </div>
        ...
        <Link href="https://wa.me/573212600682?text=Hola%2C%20quiero%20agendar%20una%20cita%20para%20cabello" target="_blank" className="block group">
